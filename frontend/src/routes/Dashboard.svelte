      ...
      {:else if activeSettingsSection === 'data' && !isMobileView}
        <PaneColumn span={1} divider={true}>
          ...
          <div class="manage-data-section manage-data-section--danger">
            ...
            <div class="form-group">
              <button type="button" class="btn-danger" on:click={() => { showDeleteDataModal = true; deleteDataConfirmText = ''; deleteDataError = ''; }}>Delete All Trip Data</button>
            </div>
          </div>
        </PaneColumn>
...
{#if showDeleteDataModal}
  <div class="delete-modal-overlay" on:click|self={() => { showDeleteDataModal = false; deleteDataConfirmText = ''; deleteDataError = ''; }}>
    <div class="delete-modal" role="dialog" aria-modal="true" aria-labelledby="delete-modal-title">
      ...
      <input
        ...
        bind:value={deleteDataConfirmText}
        placeholder="DELETE"
        autocomplete="off"
        spellcheck="false"
      />
      {#if deleteDataError}
        <p class="delete-modal-error">{deleteDataError}</p>
      {/if}
      <div class="delete-modal-actions">
        <button type="button" class="btn-secondary" on:click={() => { showDeleteDataModal = false; deleteDataConfirmText = ''; deleteDataError = ''; }} disabled={deleteDataSubmitting}>Cancel</button>
        <button
          type="button"
          class="btn-danger"
          on:click={handleDeleteAllTripData}
          disabled={deleteDataConfirmText !== 'DELETE' || deleteDataSubmitting}
        >
          {deleteDataSubmitting ? 'Deleting...' : 'Delete All Trip Data'}
        </button>
      </div>
    </div>
  </div>
